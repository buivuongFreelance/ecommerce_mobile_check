...
class ManualCallInboundScreen extends StatefulWidget {
  static const routeName = '/manual_call_outbound';
  @override
  _ManualCallInboundScreenState createState() =>
      _ManualCallInboundScreenState();
}

class _ManualCallInboundScreenState extends State<ManualCallInboundScreen> {
  ...
  Future _callVerificationCode() async {
    if (_formKey.currentState.validate()) {
      ...
      int _step = 2;
      try {
        await Device.sendVerificationInbound(
          context,
  ...
  Future _callInbound() async {
    setState(() {
      _isLoading = true;
    });
    try {
      String type = await Device.sendInbound(context, phoneNumber.toString());
